...
from calendars.functions.calendarFunctions import (
    ...
    getCurrentWeek,
    getNextHour)
from calendars.exceptions import InvalidMonthNumber
...
class TestGetNextHour(TestCase):
    def test_middle_of_day(self):
        datetimeArgument = datetime.datetime(2020,9,5,13)
        result = getNextHour(datetimeArgument)
        expectedResult = datetime.datetime(2020,9,5,14)
        self.assertEqual(result, expectedResult)

    def test_end_of_day(self):
        datetimeArgument = datetime.datetime(2020,9,5,23,30)
        result = getNextHour(datetimeArgument)
        expectedResult = datetime.datetime(2020,9,6,0,30)
        self.assertEqual(result, expectedResult)
    
    def test_no_hour_argument(self):
        datetimeArgument = datetime.datetime(2020,9,5)
        result = getNextHour(datetimeArgument)
        expectedResult = datetime.datetime(2020,9,5,1)
        self.assertEqual(result, expectedResult)

    def test_invalid_datetime_argument(self):
        self.assertRaises(TypeError, getNextHour, "datetime")